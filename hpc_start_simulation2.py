...
def start_simulation(folder_name, line_number,port_number):
	...
	if(all_set == 0):
		load_world = subprocess.Popen("export GAZEBO_MASTER_URI=http://localhost:{};gzserver --verbose sources/w_swarm1/world_db/20180209_w_swarm1_circular_uniform_litter.world".format(port_number),shell=True)#,stdin=subprocess.PIPE,stderr=subprocess.PIPE,stdout=subprocess.PIPEw_swarm1.world
