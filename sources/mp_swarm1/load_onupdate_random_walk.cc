...
void ModelVel::OnUpdate(const common::UpdateInfo & _info)
{
	...
	if(this->start_sim)
	{
		...
		{//If not in the middle of turning, litter storage not full and not seeing any litter
			...
			{//Make a random turn
				...
				this->rnd_heading = temp_heading;//save value generated by random number distribution
	...
	///////////////////////////////////////////////////////////////////////////////////////////
		...
		if(_info.simTime.nsec==0 or (this->log_timer >= this->log_rate) or true)//rate of 100Hz
		{
			this->log_timer = 0;
			...
			my_log_data = this->model->GetName() + ":" + 
							...
							to_string(fmod(this->d_heading * 180.0/M_PI + 360.0, 360.0)) + "," +
							to_string(fmod(this->rnd_heading * 180.0/M_PI + 360.0, 360.0)) + "," + 
